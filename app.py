...
def background_thread():
    ...
    while True:
        socketio.sleep(10)
        count += 1
        socketio.emit('my_response', {'data': 'Sever generated event', 'count': count}, namespace='/test')
...
def join(message):
    ...
    session['receive_count'] = session.get('receive_count', 0) + 1
    emit('my_response', {'data': f"In rooms: {', '.join(rooms())}", 'count': session['receive_count']})
...
def close(message):
    ...
    emit(
        'my_response',
        {'data': f"Room {message['room']} is closing.", 'count': session['receive_count']},
        room=message['room']
    )
